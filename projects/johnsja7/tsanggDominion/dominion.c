...
int rfAdventurer(struct gameState *state) {
  int currentPlayer = whoseTurn(state);
  int drawntreasure = 0;
  int cardDrawn;
...
int rfSmithy(struct gameState *state, int handPos) {
  int i;
  int currentPlayer = whoseTurn(state);
  for (i = 0; i < 3; i++)
    drawCard(currentPlayer, state);
...
int rfVillage(struct gameState *state, int handPos) {
  int currentPlayer = whoseTurn(state);
  drawCard(currentPlayer, state);
...
int rfSteward(struct gameState *state, int choice1, int choice2, int choice3, int handPos) {
  int currentPlayer = whoseTurn(state);
  if (choice1 == 1) {
    drawCard(currentPlayer, state);
    drawCard(currentPlayer, state);
...
int rfTreasureMap(struct gameState *state, int handPos) {
  int currentPlayer = whoseTurn(state);
  int index = -1;
  int i;
  for (i = 0; i < state->handCount[currentPlayer]; i++) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case smithy:
      return rfSmithy(state, handPos);
		
    case village:
      return rfVillage(state, handPos);
    ...
    case steward:
      return rfSteward(state, choice1, choice2, choice3, handPos);
    ...
    case treasure_map:
      return rfTreasureMap(state, handPos);
    }
